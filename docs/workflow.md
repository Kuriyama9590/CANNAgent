...
- **有效性判据（D4 门限 + 2026-09-17 追加，见 C12 #55）**：对每个选中的优化点（`strategy.selected` 的候选），依 ATC 优化清单判定其归属后二选一——**我们要做的是「＞ATC」或「ATC 未覆盖」**：
  - **ATC 已覆盖**（清单中有对应 pass 生效、overlap 命中）→ 必须**强于 ATC**：`optimized.p50_us < bench_v{N}_atc.p50_us`，否则该优化点判为无效优化（不进入交付）
  - **ATC 未覆盖**（清单中无对应 pass，或该算子/模式在 ATC 优化范围之外）→ 判为**有效**（价值来自覆盖空白，不做 ATC 对照），仍须精度达标并按基线口径报告 gain
- **ATC 优化清单（判定依据，2026-09-17 追加，见 C12 #55）**：判定前必须先**枚举 ATC 对本算子自动应用的优化**（融合/替换等 pass 及其作用范围），并逐项标注与我们优化点的对应关系——清单就是「已覆盖 / 未覆盖」分类的依据，据此把每个优化点路由到上述两支判据；清单与对照性能一并与 run 目录、交付报告同步落盘
  ```jsonc
